...
router.post('/employees', (req, res) => {
  ...
  req.db.collection('employees').insertOne({ firstName: firstName, lasName: lastName }, err => {
    if (err) res.status(500).json({ message: err });
...
router.put('/employees/:id', (req, res) => {
  const { firstName, lastName } = req.body;
  req.db.collection('employees').updateOne({ _id: ObjectId(req.params.id) }, { $set: { firstName: firstName, lasName: lastName }}, err => {
    if (err) res.status(500).json({ message: err });
    else res.json({ message: 'OK' });
  })
});
